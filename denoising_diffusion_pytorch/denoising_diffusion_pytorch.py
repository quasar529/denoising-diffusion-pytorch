...
# 각 시간 단계에 맞는 값을 가져오기 위한 함수
def extract(a, t, x_shape):
    b, *_ = t.shape  # t의 첫 번째 차원 크기를 b로 설정하고, 나머지 차원을 무시
    out = a.gather(-1, t)  # a에서 t에 해당하는 인덱스를 따라 값을 추출
...
class GaussianDiffusion(Module):
    def __init__(
        ...
        *,
        ...
        ## register_buffer에 등록하면 자동으로 cuda로 옮긴다
        ### register_buffer에 등록된 변수는 학습 X
        ...
        # 확산 과정 q(x_t | x_{t-1})와 관련된 계산 값들
        ...
        register_buffer("posterior_variance", posterior_variance)  #  q(x_{t-1} | x_t, x_0)의 분산
    ...
    def p_sample(self, x, t: int, x_self_cond=None):
        b, *_, device = *x.shape, self.device  # 배치 크기 b만큼의 타임스텝 t를 가지는 텐서를 생성
        batched_times = torch.full((b,), t, device=device, dtype=torch.long)
        model_mean, _, model_log_variance, x_start = self.p_mean_variance(
            x=x, t=batched_times, x_self_cond=x_self_cond, clip_denoised=True
        )  # 모델로부터 평균, 분산, 복원된 이미지를 예측
        noise = torch.randn_like(x) if t > 0 else 0.0  # no noise if t == 0
        pred_img = (
            model_mean + (0.5 * model_log_variance).exp() * noise
        )  # 예측된 평균과 노이즈를 이용해 다음 타임스텝의 이미지를 계산
    ...
    def p_sample_loop(self, shape, return_all_timesteps=False):
        ...
        img = torch.randn(shape, device=device)  # pure 노이즈로 시작하여 img를 초기화
        imgs = [img]
        ...
        ):  # 타임스텝 t를 역순으로 반복하며 p_sample 함수를 호출해 img 업데이트
    ...
    def q_sample(self, x_start, t, noise=None):
        ...
        )  # noise가 주어지지 않으면 x_start와 같은 형태의 랜덤 노이즈 생성
    ...
    def p_losses(self, x_start, t, noise=None, offset_noise_strength=None):
        ...
        noise = default(noise, lambda: torch.randn_like(x_start))  # randn_like -> 정규분포 따름
        ...
        x = self.q_sample(x_start=x_start, t=t, noise=noise)  # 주어진 시간 t에서 노이즈를 추가한 샘플을 생성
        ...
        model_out = self.model(x, t, x_self_cond)  # 모델을 사용하여 예측을 수행

        if self.objective == "pred_noise":  # 노이즈를 예측하는 경우
            target = noise
        elif self.objective == "pred_x0":  # x_start를 예측하는 경우
            target = x_start
        elif self.objective == "pred_v":
